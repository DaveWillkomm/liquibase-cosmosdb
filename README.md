...
<p>
<code>connectionMode</code> is optional and controls whether the underlying Cosmos Java SDK client is built with
<code>gatewayMode()</code> or <code>directMode()</code>. Accepted values are <code>gateway</code> and <code>direct</code>
(case-insensitive). When omitted, no explicit mode is set and the SDK default (<code>direct</code>) applies, preserving
prior behavior. Use <code>gateway</code> when connecting to the newer "vNext" Azure Cosmos DB Emulator, which only
